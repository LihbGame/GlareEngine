...
private:
	...
	vector<vector<InstanceRenderConstants>> mIRC;
...
void App::CreateModelInstance(ID3D12GraphicsCommandList* CommandList,string ModelName, int Num_X, int Num_Y,float offset, bool RotX)
{
	...
	{
		vector<InstanceRenderConstants> InstanceRC;
		for (int i = 0; i < Num_X; ++i)
		{
			for (int y = 0; y < Num_Y; ++y)
			{
				...
				InstanceRC.push_back(IRC);
			}
		}
		mIRC.push_back(InstanceRC);
		InstanceConstants IC;
		IC.mDataPtr = mIRC.back().data();
		IC.mDataNum = mIRC.back().size();
