...
#pragma region CommandContext
		...
		uint32_t CommandContext::ReadbackTexture(ReadbackBuffer& DstBuffer, PixelBuffer& SrcBuffer)
		{
			...
			D3D12_RESOURCE_DESC ResourceDesc = SrcBuffer.GetResource()->GetDesc();
			...
			D3D12_TEXTURE_COPY_LOCATION Dst = CD3DX12_TEXTURE_COPY_LOCATION(DstBuffer.GetResource(), PlacedFootprint);
			D3D12_TEXTURE_COPY_LOCATION Src = CD3DX12_TEXTURE_COPY_LOCATION(SrcBuffer.GetResource(), 0);
...
#pragma region GraphicsContext
		...
		void GraphicsContext::ClearDepth(DepthBuffer& Target,float ClearDepth)
		{
			FlushResourceBarriers();
			m_CommandList->ClearDepthStencilView(Target.GetDSV(), D3D12_CLEAR_FLAG_DEPTH, ClearDepth, Target.GetClearStencil(), 0, nullptr);
		}
