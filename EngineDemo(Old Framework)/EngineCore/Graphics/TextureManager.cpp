...
std::wstring  PBRTextureFileType[] = {
	L"_albedo",
	L"_normal",
	L"_ao",
	L"_metallic",
...
namespace GlareEngine
{
	namespace DirectX12Graphics
	{
		...
		void TextureManager::CreatePBRTextures(std::wstring PathName, std::vector<Texture*>& Textures)
		{
			...
			for (auto Type : PBRTextureFileType)
			{
				...
				Textures.push_back(GetModelTexture(Fullfilename, bSRGB));
			}
		}
		...
		HRESULT TextureManager::CreateD3DResources12(
			ID3D12Device* device,
		...
		HRESULT TextureManager::CreateTextureFromDDS12(
		...
		HRESULT TextureManager::CreateDDSTextureFromFile12(_In_ ID3D12Device* device,
		...
		bool TextureManager::CreateTexture(std::wstring name, std::wstring filename, bool ForceSRGB)
		{
			if (CheckFileExist(filename))
			{
				...
				ThrowIfFailed(CreateDDSTextureFromFile12(GlareEngine::DirectX12Graphics::g_Device,
