...
void SystemTime::Initialize( void )
{
    ...
    GlareAssert(ASSERT_CRITICAL,TRUE == QueryPerformanceFrequency(&frequency), L"Unable to query performance counter frequency");
...
int64_t SystemTime::GetCurrentTick( void )
{
    ...
    GlareAssert(ASSERT_CRITICAL, TRUE == QueryPerformanceCounter(&currentTick), L"Unable to query performance counter value");
