...
DXLinearAllocatorPage::DXLinearAllocatorPage() noexcept
    : pPrevPage(nullptr)
    , pNextPage(nullptr)
    , mMemory(nullptr)
    , mPendingFence(0)
...
size_t DXLinearAllocatorPage::Suballocate(_In_ size_t size, _In_ size_t alignment)
{
    ...
    if (offset + size > mSize)
    {
        ...
        throw std::runtime_error("DXLinearAllocatorPage::Suballocate");
    }
    mOffset = offset + size;
    return offset;
}

void DXLinearAllocatorPage::Release() noexcept
{
    assert(mRefCount > 0); 
...
//--------------------------------------------------------------------------------------
DXLinearAllocator::DXLinearAllocator(
    ...
    , m_device(pDevice)
{
    ...
    m_debugName = L"DXLinearAllocator";
#endif
    ...
    for (size_t preallocatePages = 0; preallocateBytes != 0 && preallocatePages < preallocatePageCount; ++preallocatePages)
    {
        ...
        {
            DebugTrace("DXLinearAllocator failed to preallocate pages (%zu required bytes, %zu pages)\n",
...
DXLinearAllocatorPage* DXLinearAllocator::FindPageForAlloc(_In_ size_t size, _In_ size_t alignment)
{
#ifdef _DEBUG
    if (size > m_increment)
...
void DXLinearAllocator::FenceCommittedPages(_In_ ID3D12CommandQueue* commandQueue)
{
    // No pending pages
    ...
    UINT numReady = 0;
    DXLinearAllocatorPage* readyPages = nullptr;
    DXLinearAllocatorPage* unreadyPages = nullptr;
    DXLinearAllocatorPage* nextPage = nullptr;
...
void DXLinearAllocator::RetirePendingPages() noexcept
{
...
void DXLinearAllocator::Shrink() noexcept
{
    FreePages(m_unusedPages);
    m_unusedPages = nullptr;
...
DXLinearAllocatorPage* DXLinearAllocator::GetCleanPageForAlloc()
{
...
DXLinearAllocatorPage* DXLinearAllocator::GetPageForAlloc(
    size_t sizeBytes,
    size_t alignment)
{
    // Fast path
...
DXLinearAllocatorPage* DXLinearAllocator::FindPageForAlloc(
    DXLinearAllocatorPage* list,
    size_t sizeBytes,
...
DXLinearAllocatorPage* DXLinearAllocator::GetNewPage()
{
    ...
    if (FAILED(hr))
    {
        if (hr != E_OUTOFMEMORY)
        {
            DebugTrace("DXLinearAllocator::GetNewPage resource allocation failed due to unexpected error %08X\n", static_cast<unsigned int>(hr));
        }
        return nullptr;
    }

#if defined(_DEBUG) || defined(PROFILE)
    spResource->SetName(m_debugName.empty() ? L"DXLinearAllocator" : m_debugName.c_str());
#endif
    ...
    auto page = new DXLinearAllocatorPage;
...
void DXLinearAllocator::UnlinkPage(DXLinearAllocatorPage* page) noexcept
{
    if (page->pPrevPage)
...
void DXLinearAllocator::LinkPageChain(DXLinearAllocatorPage* page, DXLinearAllocatorPage*& list) noexcept
{
#if VALIDATE_LISTS
    // Walk the chain and ensure it's not in the list twice
    for (DXLinearAllocatorPage* cur = list; cur != nullptr; cur = cur->pNextPage)
    {
        assert(cur != page);
    }
#endif
    ...
    DXLinearAllocatorPage* lastPage = nullptr;
...
void DXLinearAllocator::LinkPage(DXLinearAllocatorPage* page, DXLinearAllocatorPage*& list) noexcept
{
#if VALIDATE_LISTS
    // Walk the chain and ensure it's not in the list twice
    for (DXLinearAllocatorPage* cur = list; cur != nullptr; cur = cur->pNextPage)
...
void DXLinearAllocator::ReleasePage(DXLinearAllocatorPage* page) noexcept
{
...
void DXLinearAllocator::FreePages(DXLinearAllocatorPage* page) noexcept
{
    while (page != nullptr)
    {
        auto nextPage = page->pNextPage;
...
#if VALIDATE_LISTS
void DXLinearAllocator::ValidateList(DXLinearAllocatorPage* list)
{
...
void DXLinearAllocator::ValidatePageLists()
{
...
#if defined(_DEBUG) || defined(PROFILE)
void DXLinearAllocator::SetDebugName(const char* name)
{
...
void DXLinearAllocator::SetDebugName(const wchar_t* name)
{
    m_debugName = name;
...
void DXLinearAllocator::SetPageDebugName(DXLinearAllocatorPage* list) noexcept
{
    for (auto page = list; page != nullptr; page = page->pNextPage)
    {
