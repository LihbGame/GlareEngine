...
void GlareEngine::Model::AddToRender(MeshRenderPass& RenderPass, const GPUBuffer& meshConstants, const ScaleAndTranslation sphereTransforms[], const Joint* skeleton) const
{
	// Pointer to current mesh
	const uint8_t* pMesh = m_MeshData.get();

	const Frustum& frustum = RenderPass.GetViewFrustum();
	AffineTransform& viewMat = (AffineTransform&)RenderPass.GetViewMatrix();
	...
	for (uint32_t i = 0; i < m_NumMeshes; ++i)
	{
		...
		if (frustum.IntersectSphere(sphereVS))
		{
			...
			RenderPass.AddMesh(mesh, distance,
...
void GlareEngine::ModelInstance::AddToRender(MeshRenderPass& RenderPass) const
{
	if (m_Model != nullptr)
	{
		m_Model->AddToRender(RenderPass, m_MeshConstantsGPU, (const ScaleAndTranslation*)m_BoundingSphereTransforms.get(),
			m_Skeleton.get());
	}
}
