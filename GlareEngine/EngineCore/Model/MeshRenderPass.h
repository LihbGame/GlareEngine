...
namespace GlareEngine
{
	...
	public:
		...
		void AddRenderTarget(ColorBuffer& RTV)
		{
			assert(m_NumRTVs < 8);
			m_RTV[m_NumRTVs] = &RTV;
			m_RTV_Descrptor[m_NumRTVs++] = m_RTV[m_NumRTVs]->GetRTV();
		}

		void ReplaceRenderTarget(D3D12_CPU_DESCRIPTOR_HANDLE RTV,int index)
		{
			assert(index < 8);
			m_RTV_Descrptor[index] = RTV;
		}
	...
	private:
		...
		uint32_t m_NumRTVs;
		ColorBuffer* m_RTV[8];
		D3D12_CPU_DESCRIPTOR_HANDLE m_RTV_Descrptor[8] = { 0 };
		DepthBuffer* m_DSV;
	};
}
