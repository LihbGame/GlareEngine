...
void glTF::Asset::ProcessBuffers( json& buffers, ByteArray chunk1bin )
{
    ...
    for (json::iterator it = buffers.begin(); it != buffers.end(); ++it)
    {
        ...
        {
            //"Only the 1st buffer allowed to be internal"
            assert(it == buffers.begin());
            //"GLB chunk1 missing data or not a GLB file"
...
D3D12_TEXTURE_ADDRESS_MODE GLtoD3DTextureAddressMode( int32_t glWrapMode )
{
    switch (glWrapMode)
    {
    default: EngineLog::AddLog(L"Unexpected sampler wrap mode");
...
void glTF::Asset::Parse(const std::wstring& filepath)
{
    ...
    std::wstring fileExt = ToLower(GetFileExtension(filepath));
    ...
    if (fileExt == L"glb")
    {
        ...
        {
            EngineLog::AddLog(L"Error:  Invalid glTF binary format\n");
            return;
        }
        if (header.version != 2)
        {
            EngineLog::AddLog(L"Error:  Only glTF 2.0 is supported\n");
            return;
        }
        ...
        {
            EngineLog::AddLog(L"Error: Expected chunk1 to contain BIN\n");
            return;
       }
    ...
    else 
    {
        ...
        gltfFile = FileUtility::ReadFileSync(filepath);
        if (gltfFile->size() == 0)
            return;
    ...
    if (!root.is_object())
    {
        EngineLog::AddLog(L"Invalid glTF file: %s\n", filepath.c_str());
        return;
    }

    // Strip off file name to get root path to other related files
    m_basePath = GetBasePath(filepath);
