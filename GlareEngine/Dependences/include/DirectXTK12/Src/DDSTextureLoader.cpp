	...
	case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
	{
		...
		else
		{
			...
			else
			{
                ...
                CD3DX12_RESOURCE_BARRIER CopyDestPixelShader = CD3DX12_RESOURCE_BARRIER::Transition(texture.Get(),
                    D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
				cmdList->ResourceBarrier(1, &CopyDestPixelShader);
			}
		}
	} break;
	}
    ...
    _Out_opt_ DDS_ALPHA_MODE* alphaMode)
{
    ...
    HRESULT hr = LoadTextureDataFromFile(szFileName, ddsData, (const DDS_HEADER**)(&header), (const uint8_t**)(&bitData),&bitSize);
    if (FAILED(hr))
    {
        return hr;
    }
