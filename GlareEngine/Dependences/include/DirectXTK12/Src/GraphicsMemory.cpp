...
#error Unknown forward bit-scan syntax
    ...
    public:
            ...
            : mDevice(device)
        {
            ...
            for (size_t i = 0; i < mPools.size(); ++i)
            {
                ...
                mPools[i] = std::make_unique<DXLinearAllocator>(
                    mDevice.Get(),
                    pageSize);
            }
        }
        ...
        // Explicitly destroy DXLinearAllocators inside a critical section
        ~DeviceAllocator()
        {
    ...
    private:
        ...
        std::array<std::unique_ptr<DXLinearAllocator>, AllocatorPoolCount> mPools;
...
GraphicsResource::GraphicsResource(
    _In_ DXLinearAllocatorPage* page,
