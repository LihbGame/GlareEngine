...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& Pos, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImRect bb(ImagePosition, ImagePosition + size);
...
bool ImGui::RadioButton(const char* label, bool active, ImVec2 offset)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos+ offset;
    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, &hovered, &held, ImGuiButtonFlags_PressedOnClickRelease);
    if (pressed)
        MarkItemEdited(id);
...
IMGUI_API bool ImGui::SliderVerticalScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
IMGUI_API bool ImGui::SliderVerticalFloat(const char* label, float* v, float v_min, float v_max, const char* format, float power)
{
	ImGui::Text(label);
    return SliderVerticalScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
