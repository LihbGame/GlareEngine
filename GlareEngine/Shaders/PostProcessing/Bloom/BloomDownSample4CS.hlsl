...
RWTexture2D<float3> DownSampleResult8x8 : register(u0);
RWTexture2D<float3> DownSampleResult4x4 : register(u1);
RWTexture2D<float3> DownSampleResult2x2 : register(u2);
RWTexture2D<float3> DownSampleResult1x1 : register(u3);
...
void main(uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID)
{
    ...
    g_Tile[GI] = AvgPixel;
    DownSampleResult8x8[DTid.xy] = AvgPixel;
    ...
    if ((Parity & 3) == 0)
    {
        ...
        g_Tile[GI] = AvgPixel;
        DownSampleResult2x2[DTid.xy >> 2] = AvgPixel;
    }
